...
function ExpectFolderPath(folderPath, onFail) {
    ...
    (0, Expect_1.Expect)(folderPath === "" || folderPath.endsWith("/"), `data: Must end with "/" if it's not the local directory folder (""): ` + folderPath);
    (0, Expect_1.Expect)(!folderPath.includes("*"), `data: Cannot have "*": ` + folderPath);
}
...
function MoveFileSystemPathIntoFolderPath(fileSystemPath, toFolderPath) {
    ...
    (0, Expect_1.Expect)(name, `Cannot move a path that has no name. Did you accidentally try to move the root folder?`);
